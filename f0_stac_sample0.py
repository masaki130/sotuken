# f0_stac_result_5000_notaccentに保存
# 各音節における統計値を計算し、保存
# 実行：python f0_stac_sample0.py --num_files 3
...
def save_statistics_csv(filename, statistics):
    """Save the F0 statistics (mean, max, min, std, median) into a CSV file."""
    with open(filename, 'w', newline='') as csvfile:
        fieldnames = ['Mora', 'Mean', 'Max', 'Min', 'Std', 'Median']
...
def savefig(filename, f0, frame_period, alignment):
    ...
    for start, end, label in alignment:
...
def main(args):
    ...
    result_dir = 'heikin_result_5000'
    csv_dir = 'f0_stac_result_5000_notaccent'
    ...
    for i, alignment_file in enumerate(alignment_files):
        ...
        statistics = savefig(f'{result_dir}/f0_heikin_result_{i+1}.png', f0, new_frame_period, alignment)
        
        # Save the statistics to a CSV file
        save_statistics_csv(f'{csv_dir}/f0_statistics_{i+1}.csv', statistics)
